...
void run_tests() {
  ...
  volatile void (**last_tcs)(void) =
      &__start_test_function_pointers + opts.threads_num * tc_per_thread_num;
  uint64_t tc_leftover =
      (opts.threads_num)
          ? (&__stop_test_function_pointers - &__start_test_function_pointers) %
                opts.threads_num
          : &__stop_test_function_pointers - &__start_test_function_pointers;

  auto thread_task = [](volatile void (**start_addr)(void),
                        uint64_t num) -> void {
  ...
  for (uint64_t i = 0; i < opts.threads_num; i++) {
    ...
    threads[i] = std::thread(
        thread_task, &__start_test_function_pointers + (i * tc_per_thread_num),
        tc_per_thread_num);
  }
...
void assert_str_equal(const char *exp1, const char *exp2, const char *file,
  ...
  if (!ok) {
    ...
    if constexpr ((!std::is_null_pointer_v<decltype(exp1)> &&
                   !std::is_null_pointer_v<decltype(exp2)>))
      if (opts.verbose_level > 1) {
  ...
  report.at(ts_name).at(tc_name).push_back(
      std::make_tuple(ok, "ASSERT_STREQ", exp1_str, exp2_str));
  asserts_counter++;
...
void assert_not_str_equal(const char *exp1, const char *exp2, const char *file,
  ...
  test_results.push_back(
      std::make_tuple(asserts_counter, ok, ts_name, tc_name,
                      ...
                      std::string(exp1_str), std::string(exp2_str)));
  report.at(ts_name).at(tc_name).push_back(
      std::make_tuple(ok, "ASSERT_NOT_STREQ", exp1_str, exp2_str));
  asserts_counter++;
...
void expect_not_str_equal(const char *exp1, const char *exp2, const char *file,
  ...
  test_results.push_back(
      std::make_tuple(asserts_counter, ok, ts_name, tc_name,
                      ...
                      std::string(exp1_str), std::string(exp2_str)));
  report.at(ts_name).at(tc_name).push_back(
      std::make_tuple(ok, "EXPECT_NOT_STREQ", exp1_str, exp2_str));
  asserts_counter++;
}
...
void print_results(void) {
  ...
  for (const std::pair<
    ...
    for (const std::pair<std::string,
                         std::vector<std::tuple<bool, std::string, std::string,
                                                std::string>>> &testcase_info :
         testsuite_info.second) {
      ...
      for (const std::tuple<bool, std::string, std::string, std::string>
               &check_info : testcase_info.second) {
        if (!std::get<1>(check_info).empty()) {

          if (opts.verbose_level > 0)
            std::printf((std::get<0>(check_info))
                            ? "\t\t\t[\e[32mOK\e[39m] (%s), ( \e[33m%s\e[39m, "
                              "\e[33m%s\e[39m )\r\n"
                            : "\t\t\t[\e[31mFAIL\e[39m] (%s), ( "
