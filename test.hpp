...
void assert_equal(A exp1, B exp2, const char *file, int line, const char *exp1_str, const char *exp2_str) {
...
template <typename A, typename B>
void assert_not_equal(A exp1, B exp2, const char *file, int line, const char *exp1_str, const char *exp2_str) {
  success_t ok = (exp1 != exp2);
  std::lock_guard<std::mutex> lock(mtx);
...
template <typename A, typename B>
void expect_equal(A exp1, B exp2, const char *file, int line, const char *exp1_str, const char *exp2_str) {
  success_t ok = (exp1 == exp2);
  std::lock_guard<std::mutex> lock(mtx);
...
template <typename A, typename B>
void expect_not_equal(A exp1, B exp2, const char *file, int line, const char *exp1_str, const char *exp2_str) {
  success_t ok = (exp1 != exp2);
  std::lock_guard<std::mutex> lock(mtx);
    ...
    if (opts.verbose_level > 1)                                                                                        \
      std::printf("#%lu [\e[31mFAIL\e[39m] At %s:%i due to std exception ( "                                           \
                  ...
                  asserts_counter, __FILE__, __LINE__, e.what());                                                      \
                                                                                                                       \
    asserts_counter++;                                                                                                 \
    test_results.push_back(std::make_tuple(asserts_counter, false, ts_name, tc_name,                                   \
                                           std::string(__FILE__) + ":" + std::to_string(__LINE__), std::string(#A),    \
                                           std::string(#B)));                                                          \
...
#define ASSERT_NOT_EQ(A, B)                                                                                            \
  try {                                                                                                                \
    ...
    report.at(ts_name).at(tc_name).push_back(                                                                          \
        std::make_tuple(false, "ASSERT_NOT_EQ", std::string(#A), std::string(#B)));                                    \
    std::terminate();                                                                                                  \
  }                                                                                                                    \
  if (opts.verbose_level > 1)                                                                                          \
  std::cout << ""
...
#define EXPECT_NOT_EQ(A, B)                                                                                            \
  try {                                                                                                                \
    ...
    test_results.push_back(std::make_tuple(asserts_counter, false, ts_name, tc_name,                                   \
                                           ...
                                           std::string(#B)));                                                          \
    report.at(ts_name).at(tc_name).push_back(                                                                          \
        std::make_tuple(false, "EXPECT_NOT_EQ", std::string(#A), std::string(#B)));                                    \
    std::terminate();                                                                                                  \
  }                                                                                                                    \
  if (opts.verbose_level > 1)                                                                                          \
  std::cout << ""
...
#define ASSERT_STREQ(A, B)                                                                                             \
  try {                                                                                                                \
    ...
    test_results.push_back(std::make_tuple(asserts_counter, false, ts_name, tc_name,                                   \
                                           ...
                                           std::string(#B)));                                                          \
    report.at(ts_name).at(tc_name).push_back(                                                                          \
        std::make_tuple(false, "ASSERT_STREQ", std::string(#A), std::string(#B)));                                     \
    std::terminate();                                                                                                  \
  }                                                                                                                    \
  if (opts.verbose_level > 1)                                                                                          \
  std::cout << ""
...
#define ASSERT_NOT_STREQ(A, B)                                                                                         \
  try {                                                                                                                \
    ...
    test_results.push_back(std::make_tuple(asserts_counter, false, ts_name, tc_name,                                   \
                                           ...
                                           std::string(#B)));                                                          \
    report.at(ts_name).at(tc_name).push_back(                                                                          \
        std::make_tuple(false, "ASSERT_NOT_STREQ", std::string(#A), std::string(#B)));                                 \
    std::terminate();                                                                                                  \
  }                                                                                                                    \
  if (opts.verbose_level > 1)                                                                                          \
  std::cout << ""
...
#define EXPECT_STREQ(A, B)                                                                                             \
  try {                                                                                                                \
    ...
    test_results.push_back(std::make_tuple(asserts_counter, false, ts_name, tc_name,                                   \
                                           ...
                                           std::string(#B)));                                                          \
    report.at(ts_name).at(tc_name).push_back(                                                                          \
        std::make_tuple(false, "EXPECT_STREQ", std::string(#A), std::string(#B)));                                     \
    std::terminate();                                                                                                  \
  }                                                                                                                    \
  if (opts.verbose_level > 1)                                                                                          \
  std::cout << ""
...
#define EXPECT_NOT_STREQ(A, B)                                                                                         \
  try {                                                                                                                \
    ...
    test_results.push_back(std::make_tuple(asserts_counter, false, ts_name, tc_name,                                   \
                                           ...
                                           std::string(#B)));                                                          \
    report.at(ts_name).at(tc_name).push_back(                                                                          \
        std::make_tuple(false, "EXPECT_NOT_STREQ", std::string(#A), std::string(#B)));                                 \
    std::terminate();                                                                                                  \
  }                                                                                                                    \
  if (opts.verbose_level > 1)                                                                                          \
  std::cout << ""
